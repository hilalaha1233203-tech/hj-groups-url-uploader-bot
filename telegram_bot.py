...
async def load_access_state():
    ...
    # First boot migration: existing allowlisted users are preserved in the
    # durable access table. Future deploys no longer depend on an env allowlist.
...
async def load_destination(uid):
    ...
    if saved:
        ...
        return saved
    # One-time migration of old local/env configuration into durable storage.
...
async def select_cmd(message: Message):
    ...
    try: ids = {int(value.strip()) for value in parts[1].split(",") if value.strip()}
...
async def button_session(message: Message): return await session_cmd(message.model_copy(update={"text": "/session"}))
@dp.message(F.text == "🔗 Scan Link")
async def button_scan(message: Message):
    uid = message.from_user.id if message.from_user else None
    if not allowed(uid): return
    PENDING[uid] = "scan"; await message.answer("🔗 Send a Telegram link. For bulk use .../471-480 or .../471 480.", reply_markup=menu())
@dp.message(F.text == "📦 Bulk Range")
async def button_range(message: Message): return await range_cmd(message.model_copy(update={"text": "/range"}))
@dp.message(F.text == "🎯 Destination")
async def button_destination(message: Message): return await setdest_cmd(message.model_copy(update={"text": "/setdestination"}))
@dp.message(F.text == "📋 Select Files")
async def button_select(message: Message): return await select_cmd(message.model_copy(update={"text": "/select"}))
@dp.message(F.text == "❌ Cancel")
async def button_cancel(message: Message): return await cancel_cmd(message.model_copy(update={"text": "/cancel"}))
@dp.message(F.text == "🚪 Logout")
async def button_logout(message: Message): return await logout_cmd(message.model_copy(update={"text": "/logout"}))
@dp.message(F.text == "ℹ️ Help")
async def button_help(message: Message): return await help_cmd(message.model_copy(update={"text": "/help"}))
...
async def text_handler(message: Message):
    ...
    if action == "range": return await range_cmd(message.model_copy(update={"text": f"/range {text}"}))
    if action == "select": return await select_cmd(message.model_copy(update={"text": f"/select {text}"}))
    if action == "scan":
        PENDING.pop(uid, None)
        if not re.match(r"^https?://(?:www\.)?t\.me/", text): return await message.answer("Send a valid Telegram t.me message link.", reply_markup=menu())
...
async def pick(callback: CallbackQuery):
    ...
    _, owner, typ = callback.data.split(":", 2); job = JOBS.get(int(owner))
...
async def individual(callback: CallbackQuery):
    ...
    job.selected = set(); await callback.message.edit_text("Select individual files.\n\nSelected: 0", reply_markup=individual_menu(job)); await callback.answer()

@dp.callback_query(F.data.startswith("toggle:"))
async def toggle(callback: CallbackQuery):
    uid = callback.from_user.id
    if not allowed(uid): return await callback.answer("Not authorized", show_alert=True)
    _, owner, mid_text = callback.data.split(":", 2); mid = int(mid_text); job = JOBS.get(int(owner))
    ...
    await callback.message.edit_reply_markup(reply_markup=individual_menu(job)); await callback.answer(f"Selected: {len(job.selected)}")

@dp.callback_query(F.data.startswith("individual_confirm:"))
async def individual_confirm(callback: CallbackQuery):
    uid = callback.from_user.id; job = JOBS.get(int(callback.data.split(":", 1)[1]))
    if not allowed(uid): return await callback.answer("Not authorized", show_alert=True)
    if not job or job.owner_id != uid: return await callback.answer("Job expired", show_alert=True)
...
async def confirm(callback: CallbackQuery):
    ...
    try: await process(job, callback.message)
...
async def main():
    ...
    for uid in ALLOWED_USER_IDS: await load_destination(uid)
    session = await saved_session()
    if session:
        await rebuild(session)
        if await user_client.is_user_authorized():
