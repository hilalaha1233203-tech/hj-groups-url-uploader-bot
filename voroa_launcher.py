...
The worker is intentionally self-contained: startup failures are logged and
retried, while permanent configuration failures remain explicit.  The HTTP
health listener is opt-in because Render Background Workers do not require a
listening port.
"""
    ...
    try:
        ...
        while True:
            try:
                ...
                print(
                    f"[Voroa] Startup failed #{startup_attempt}: {type(exc).__name__}: {exc}",
                    flush=True,
                )
                if startup_attempt >= STARTUP_RETRY_LIMIT:
                    raise RuntimeError(
                        f"Voroa startup failed after {STARTUP_RETRY_LIMIT} attempts. "
                        "Check Telegram credentials, bot token, and deployment environment variables."
                    ) from exc
