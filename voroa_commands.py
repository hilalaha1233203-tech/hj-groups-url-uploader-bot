"""Extra user-facing commands for the HJ GROUPS OF FILES bot.

Kept separate from the transfer engine so command/UI additions cannot disturb
Telegram media-transfer lifecycle code.
"""
...
async def cmd_settings(message: Message) -> None:
    ...
    await message.answer(
        ...
        "Use 🎯 Destination to change the target chat/channel, or 🔐 Login to change the Telegram account.",
        parse_mode="HTML",
        reply_markup=v.menu(),
    )
...
async def cmd_features(message: Message) -> None:
    ...
    await message.answer(
        ...
        "🔐 Telegram user login with 2FA support\n"
        "📱 Session status\n"
        "⚙️ Settings and persistent destination\n\n"
        "Use /settings to view the current configuration.",
        parse_mode="HTML",
        reply_markup=v.menu(),
    )
